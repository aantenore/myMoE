...
Audit trail logging lives in `src/local_moe/audit.py`. The web host appends JSONL events for sensitive actions and exposes a filtered recent-event view through `/api/audit`. Audit metadata is deliberately operational: action, status, risk class, subject id, counts, and short error messages. It does not store prompt text, chat transcript text, memory text, environment variables, or model log bodies.
