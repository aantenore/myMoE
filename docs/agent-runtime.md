...
| Generation Run Log | Records successful generation observations for tuning and troubleshooting. | Writes metadata only to `<runtime.work_dir>/runs.jsonl`: prompt hash, prompt length, selected experts, model ids, latency, token counts, context pressure, memory ids, and error counts. It never stores prompt text or answer text. Retention pruning requires explicit confirmation. | CLI `--runs`, CLI `--runs-prune`, web `/api/runs`, web `/api/runs/prune`, Advanced Run Log panel. |
