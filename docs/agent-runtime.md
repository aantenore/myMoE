...
| Support Bundle | Exports a privacy-safe diagnostic bundle for issue reports or handoff. | Read-only; excludes chat transcripts, memory records, environment variables, secrets, and log contents. | CLI `--support-bundle`, web `/api/support-bundle`, web `/api/support-bundle/download.json`, Advanced System Doctor panel. |
