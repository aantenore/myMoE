...
| Support Bundle | Exports a privacy-safe diagnostic bundle for issue reports or handoff. | Read-only; includes Doctor, Environment Snapshot, performance, runtime optimizer summary, storage capacity summary, quality gate, hardware, paths, model log paths, and the generation run log path; excludes chat transcripts, memory records, generation run log contents, environment variables, secrets, benchmark response excerpts, and log contents. | CLI `--support-bundle`, web `/api/support-bundle`, web `/api/support-bundle/download.json`, Advanced System Doctor panel. |
