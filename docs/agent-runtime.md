...
| CLI chat sessions | Creates and continues local chat sessions without the web UI. | Uses the same `<runtime.work_dir>/chats.json`, context policy, memory retrieval, route/generation separation, and metadata-only run log as the web UI. Deletion requires `--chat-confirm`. | CLI `--interactive`, `--new-chat`, `--chat-session`, `--list-chats`, `--export-chat`, `--rename-chat`, `--delete-chat --chat-confirm`. |
