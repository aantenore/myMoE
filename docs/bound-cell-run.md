...
content-addressed, **metadata-only receipt**. The receipt may contain hashes,
identifiers, timestamps, sizes, status, counters, and drift reason codes; it must
not contain the task text or model response.
...
answer goes to standard output. `--receipt-out` creates a new private receipt
...
receipt remains safe to inspect without becoming a transcript store.
...
metadata receipt is durably appended before canonical no-clobber publication.
...
unknown-delivery marker or the finalized receipt for recovery.
...
flowchart TD
  ...
  A -->|"verified"| P["Final fresh Advisor preview and cross-binding"]
  P -->|"blocked or changed"| B
  P -->|"passed"| G1["GET /models pre-probe"]
  G1 -->|"expected model present"| C["One POST /chat/completions inference attempt"]
  C --> G2["GET /models post-probe"]
  G2 --> Q["Post-run Bound Cell inspection"]
  Q --> R["Compare sampled stable identities"]
  R --> O["Return answer separately; emit metadata-only receipt"]
```
...
4. strictly reload the source Advisor receipt, verify every nested digest, and
   run the fresh preview against the exact task, catalog, evaluation contract,
   policy, and current resources as the final admission step before endpoint
   traffic;
5. require the same selected `cell_id` and passport digest, `compute_only`, and
   zero tool surfaces; and
6. require the configured uncredentialed HTTP endpoint to use a literal numeric
...
Only then does the runner issue the first `GET /models` and require the exact
configured model ID. It next makes exactly one OpenAI-compatible
`POST /chat/completions` inference attempt and finally issues the second
`GET /models`. Thus a complete attempted path contains three HTTP requests but
only one inference. There is no retry, fallback, speculative call, judge call,
semantic cache lookup, alternate candidate, or hidden router call in v1. A
transport failure is recorded honestly; it does not trigger a second inference
attempt.
...
| Bound Cell Run | Pre-inspect, run the final preview, perform two model probes around one guarded inference attempt, then sample bindings again. | Manage or attest model residency, tools, agent loops, or arbitrary workflows. |
...
- reserve RAM, unified memory, or accelerator memory;
...
credentials in endpoint configuration, declared tools, or non-compute risk.
...
or may have completed; they must never be reported as a clean pre-call block or
...
3. bounded inspection of model, runtime, harness, tool-contract, configuration,
   and endpoint lineage;
4. one deliberately capability-poor execution;
5. pre/post drift evidence; and
6. a replayable metadata-only receipt that keeps the answer separate.
...
versioned contracts. None of those stronger claims belongs in v1.
...
- ambiguous transport failures remain ambiguous and are never retried;
- an interruption is post-checked, receipted, durably published, and only then
  ...
  journal;
- all receipt digests verify after round-trip serialization; and
