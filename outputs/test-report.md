...
- `tests/test_model_inventory.py`: read-only model asset inventory for Hugging Face snapshots, local GGUF files, deduplicated repeated model requests, missing-file recommendations, and size summaries.
...
- `tests/test_support_bundle.py`: privacy-safe diagnostic bundle content and exclusions, including embedded environment snapshot, storage metadata, model asset inventory, and MCP env name/value omission.
- `tests/test_cli.py`: eval mode, setup readiness, profile recommendation, profile preparation, profile activation, startup runbook preview and confirmation guard, performance report output, environment snapshot output, Security Audit output and Markdown rendering, generation run log listing with metadata-only summary and confirmation-guarded pruning, guarded runtime preparation preview, model process status, sanitized model log tail output, doctor output, prompt mode through the public CLI, persistent one-shot CLI chat sessions, persistent interactive CLI context reuse, CLI chat search, and guarded CLI chat compaction.
...
- `tests/test_scheduler.py`: cron dry runs, allowlisted actions including storage inspection, unsupported command rejection, write-local confirmation, expired-memory pruning jobs, auto-runnable filtering, and background runner status.
...
Public extension registry payloads now omit MCP env names and values. The runtime registry retains the original values for local MCP process startup, while UI, CLI, Doctor, and support bundle responses expose only `env_configured` and `env_count`.

Security Audit now reports permission posture, process execution, model endpoint locality, MCP env counts, MCP allowlist coverage, cron write-risk automation, enabled write-risk tools, plugin risk classes, and extension registry health from CLI, API, UI, support bundle, and the allowlisted `security.audit` tool. Its payloads are metadata-only and exclude env names/values, user content, MCP tool results, local data bundles, and logs.
