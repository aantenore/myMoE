...
- `tests/test_cli.py`: eval mode, setup readiness, profile recommendation, profile preparation, profile activation, startup runbook preview and confirmation guard, performance report output, environment snapshot output, guarded runtime preparation preview, model process status, sanitized model log tail output, doctor output, and prompt mode through the public CLI.
...
- compileall: passed
- unit/contract tests: `216/216` passed
