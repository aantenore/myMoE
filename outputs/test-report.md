...
- `tests/test_cli.py`: eval mode, setup readiness, profile recommendation, profile preparation, profile activation, performance report output, environment snapshot output, guarded runtime preparation preview, model process status, sanitized model log tail output, doctor output, and prompt mode through the public CLI.
...
- compileall: passed
- unit/contract tests: `210/210` passed
