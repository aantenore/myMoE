...
class WebTests(unittest.TestCase):
    ...
    def test_serves_runtime_and_extensions(self) -> None:
        ...
        try:
            ...
            about = _get_json(base_url + "/api/about")
            about_markdown = _get_text(base_url + "/api/about/report.md")
            ...
            security = _get_json(base_url + "/api/security")
            security_markdown = _get_text(base_url + "/api/security/report.md")
            smoke = _post_json(
        ...
        self.assertEqual(about["schema_version"], "1.0")
        self.assertEqual(about["paths"]["moe_config"], "tests/fixtures/moe.synthetic.json")
        self.assertIn("python", about)
        self.assertIn("packages", about)
        self.assertEqual(about["storage"]["schema_version"], "1.0")
        self.assertIn(about["storage"]["status"], {"ready", "attention"})
        self.assertEqual(about["runtime"]["expert_count"], 3)
        self.assertIn("# myMoE Environment Snapshot", about_markdown)
        self.assertIn("`synthetic-general`", about_markdown)
        self.assertGreaterEqual(profiles["count"], 1)
        self.assertIn("hardware", profiles)
        self.assertIn("recommended_strategy", profiles["hardware"])
        ...
        self.assertIn("recommendation", profiles)
        self.assertEqual(recommendation["recommendation"]["profile_path"], profiles["recommendation"]["profile_path"])
        self.assertGreaterEqual(recommendation["profiles_considered"], 1)
        active_profile = next(item for item in profiles["profiles"] if item["active"])
        self.assertIn("hardware_fit", active_profile)
        self.assertIn("launch_commands", active_profile)
        self.assertIn("start_ui", {command["id"] for command in active_profile["launch_commands"]})
        ...
        self.assertIn("storage", doctor)
        self.assertIn("storage", {item["id"] for item in doctor["checks"]})
        self.assertIn("# myMoE System Doctor Report", doctor_markdown)
        self.assertIn("`hardware_fit`", doctor_markdown)
        self.assertEqual(security["schema_version"], "1.0")
        self.assertEqual(security["status"], "ready")
        self.assertEqual(security["model_endpoints"]["remote_count"], 0)
        self.assertIn("# myMoE Security Audit", security_markdown)
        ...
        self.assertEqual(support_bundle["environment"]["schema_version"], "1.0")
        self.assertIn("chat transcripts", support_bundle["privacy"]["excludes"])
        self.assertEqual(downloaded_bundle["schema_version"], "1.0")
        self.assertIn("runtime_optimizer", support_bundle)
        self.assertEqual(support_bundle["runtime_optimizer"]["mode"], "read_only")
        self.assertIn("security_audit", support_bundle)
        ...
        self.assertIn(optimizer["status"], {"ready", "watch", "attention"})
        self.assertEqual(optimizer["mode"], "read_only")
        self.assertIn("signals", optimizer)
    ...
    def test_persists_chat_sessions_over_web_api(self) -> None:
        ...
        self.assertEqual(runs["summary"]["record_count"], 2)
        self.assertGreaterEqual(runs["summary"]["latency_ms"]["count"], 2)
        self.assertGreaterEqual(len(runs["summary"]["experts"]), 1)
    ...
    def test_ui_supports_markdown_rendering_and_enter_shortcut(self) -> None:
        ...
        self.assertIn("fit-status", html)
        self.assertIn("profile.launch_commands", html)
        self.assertIn("Launch hints", html)
        self.assertIn("command-line", html)
        self.assertIn("data-copy-profile-command", html)
        self.assertIn("data-command-index", html)
        self.assertIn("copyProfileCommand", html)
        self.assertIn("navigator.clipboard", html)
        self.assertIn("permission is denied", html)
        ...
        self.assertIn("renderAbout", html)
        self.assertIn("Cache free", html)
        self.assertIn("about?.storage", html)
        self.assertIn("downloadAboutReport", html)
        self.assertIn("/api/cron/run", html)
        self.assertIn("cron-confirm-writes", html)
        self.assertIn("/api/doctor", html)
        self.assertIn("/api/doctor/report.md", html)
        self.assertIn("System Doctor", html)
        self.assertIn("runDoctor", html)
        self.assertIn("downloadDoctorReport", html)
        ...
        self.assertIn("/api/security", html)
        self.assertIn("/api/security/report.md", html)
        self.assertIn("Security", html)
        self.assertIn("renderSecurity", html)
        self.assertIn("refreshSecurity", html)
        ...
        self.assertIn("/api/runtime/optimizer", html)
        self.assertIn("/api/runtime/optimizer/report.md", html)
        self.assertIn("Runtime Optimizer", html)
        self.assertIn("renderRuntimeOptimizer", html)
        self.assertIn("refreshRuntimeOptimizer", html)
